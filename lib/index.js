...
class FileRestructure {
  constructor(config = {}) {
    this.config = {
      ...
      extensions: config.extensions || ['.ts'],
      assetsDirName: config.assetsDirName || 'assets',
  ...
  getMessages() {
    const messages = {
      ...
      zh: {
        startRestructuring: '开始文件结构调整...',
        sourceDirNotExist: '错误: 源目录不存在:',
        referenceDirNotExist: '错误: 参考目录不存在:',
        gettingReferenceFiles: '获取参考文件...',
        foundReferenceFiles: '找到参考文件:',
        gettingSourceFiles: '获取源文件...',
        foundSourceFiles: '找到源文件:',
        processingRemainingFiles: '处理剩余文件...',
        restructuringComplete: '文件结构调整完成！共复制文件到:',
        copiedFiles: '总复制文件数:',
        generatedStructure: '生成的目录结构:',
        copyingFile: '复制文件:',
        to: '到'
      }
    };
    
    return messages[this.config.language] || messages.en;
  }
    ...
    function traverse(currentPath) {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          if (self.config.extensions.includes(ext)) {
  ...
  restructure() {
    ...
    // 创建assets目录
    const assetsDir = path.join(this.config.outputDir, this.config.assetsDirName);
    ...
    for (const refFile of referenceFiles) {
      ...
      if (matchingFiles.length > 0) {
        // 创建目标目录结构
        const targetDir = path.join(assetsDir, refDir);
